...
- **Real-time Logs**: View service logs as they happen. ANSI escape sequences, including color/style plus cursor moves and clear-line/clear-screen codes, are stripped, since logs are rendered as plain text
...
| `crashed`      | The process died abnormally because it was killed by a crash signal (e.g. `SIGSEGV`), exited with a code ≥ `128`, or was force-killed (`SIGKILL`) from _outside_ the dashboard while running. (A dashboard-initiated stop that escalates to `SIGKILL` settles as `stopped`, and a start that times out and is torn down settles as `error`, so you only see `crashed` from a `SIGKILL` the dashboard didn't send.) |
