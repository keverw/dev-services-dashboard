...
- **Log reads expose the ring buffer honestly**: `GET …/logs` supports `limit` (the tail, default 100), `cursor`, `since`, `logType`, and `format=text`, and reports `bufferSize`, `bufferLimit`, and `truncated` so a poller knows when older lines have already been evicted. Multi-line output chunks are expanded so every printed line carries its own timestamp prefix and stays greppable.
  `truncated` now means "lines you may never have read are gone", which is how a poller was already meant to read it: it covers a `DELETE …/logs` throwing a non-empty buffer away, and a cursor from before a dashboard restart (the counter lives on the `ServiceManager`, so it starts again at 1; such a cursor is served the buffer from the start rather than being stranded past the end of it), as well as ring-buffer eviction.
  `dsd logs <service> --cursor <seq>` exposes the same paging to the CLI, for a script or agent that can't hold a `--follow` stream open: each `--json` response carries the `nextCursor` to pass to the next call, so every new line is seen exactly once. Like `--since`, it queries the stored buffer and so can't be combined with `--follow`.
...
- Fixed an oversized `graceMs` producing the opposite of what it asked for. `setTimeout` holds its delay in a 32-bit signed integer, so anything above `2147483647` was silently clamped by Node to 1ms, meaning `--grace 3000000000` SIGKILLed the service almost immediately instead of waiting. The HTTP API now rejects an out-of-range `graceMs` with `400`, the WebSocket drops it (falling back to the configured `stopTimeout`), and the timer itself is clamped as a backstop.
- Fixed Ctrl+C being swallowed on every command except `logs --follow`. The CLI installs its own SIGINT/SIGTERM handlers, but the abort signal only reached the follow path, so a blocking `dsd start` or a `dsd stop` waiting out a long grace period (both run with no client deadline) ignored the first press and needed a second. The signal now cancels the in-flight HTTP request too, and a command cut short before it finished exits with the conventional `130`, reporting `{"error":{"code":"interrupted"}}` under `--json` rather than claiming the dashboard was unreachable. `logs --follow` is unaffected and still exits `0`, since ending a follow is what Ctrl+C is for there.
- Fixed `--timeout` not covering the response body. The deadline was cleared as soon as `fetch` resolved, which happens when the headers arrive, so a dashboard that stalled mid-body left the CLI blocked indefinitely past the timeout it was given.
- Fixed `dsd status` column alignment when color is on. Status cells are colorized before the table is laid out, and widths were measured with `String.length`, which counts ANSI escape sequences as visible characters; since the green `running` code is one byte longer than the dim `stopped` one, every column after STATUS shifted by a character. Widths are now measured on the escape-stripped text.
- Fixed a malformed percent-escape in a service id (e.g. `GET /api/v1/services/%ZZ`) being reported as a logged `500 internal_error`. `decodeURIComponent` throws on it, which is a caller mistake, so it now returns `400 bad_request`.
- Fixed a plain stop against a service already `stopping` coming back as `409 stop_failed` (exit `3`) while the stop was in fact proceeding perfectly normally. Only `--force` and `--grace` joined the stop already in flight; without either, `stopService` returned immediately, so the route saw the service still `stopping` and called it a failure. Any stop now joins the in-flight one and settles with it, which is what "stop this service" means. (`--force` / `--grace` additionally retune it, as before.)
- Fixed the header's Stop All progress toast being stranded on screen for the rest of the session. Pressing **Force Stop All** during a Stop All run starts a second run, and its `stop_all_begin` overwrote the reference to the first run's toast. That toast is sticky (it lives until explicitly removed, and is exempt from the on-screen toast cap), so nothing could ever dismiss it. A new run now retires the previous progress toast first.
- Fixed `dsd help` claiming `--timeout` defaults to 15000ms for `stop`. It has no client-side deadline, like the other lifecycle commands, since a stop waits out a configurable `stopTimeout`. The same text is served from `dsd help --json`, so an agent inherited it.
- Fixed `--grace` being silently discarded against a service already stuck in `stopping`. Only `--force` reached the stop already in flight; a grace override started no second stop, changed nothing, and came back as `409 stop_failed` (exit `3`). It now re-arms the in-flight stop's grace period, measured from the moment of the request, and settles with that stop like `--force` does. `stop-all --grace` likewise sweeps up wedged services, which previously only `--force` did.

- Fixed the exported control-API types omitting `waited`. `POST …/start` and `…/restart` always return it, but the published `ServiceResponse` only declared `ok` and `service`, so a TypeScript consumer typing its client against the package couldn't read a field the server really sends (and couldn't tell a `202` fire-and-forget summary, captured before the work ran, from a waited one). Those two routes now have their own `LifecycleResponse` (`ServiceResponse` plus `waited`), exported alongside it; `stop` keeps `ServiceResponse`, since it always waits.

### Tests

- Added `api-router.test.ts` (35 tests) covering the route table, the error envelope and status codes, blocking vs. `wait:false` starts, signal refusals (`422` undeclared vs. `409` not running), log `limit`/`since`/`logType`/`format=text`, the `415`/`403` guards, and that `GET /api/services-config` is byte-compatible. Includes a regression test asserting no response ever contains `env`, `command`, `cwd`, or the process handle; the internal `Service` holds a live `ChildProcess` and the resolved environment, so every response is built through an explicit mapper rather than by serializing it.
- Added `run.test.ts` (50 tests) driving the CLI against a real dashboard, asserting exit codes and captured stdout/stderr. All CLI logic lives in `run(argv, io)` with output and environment injected, so no test spawns a child process.
...
- Added `ServiceManager` tests for the two `graceMs` fixes: a re-armed grace period escalating a stop already in flight (and settling its original caller), and an oversized override not collapsing the grace period to an immediate kill. Both run on real timers, since the suite-wide `setTimeout` mock collapses long delays and would erase the very distinction each test exists to prove.
- Added CLI tests for the interrupt contract: a request cut short mid-flight exits `130` rather than `5` (driven against a server that accepts the connection and never answers), the `--json` envelope's `exitCode` agreeing with the process exit code, and `finalExitCode` keeping a completed command's own code so `logs --follow` still exits `0` on Ctrl+C.
